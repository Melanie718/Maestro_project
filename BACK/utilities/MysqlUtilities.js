...
class MysqlUtilities
{
    ...
    //                                  CONNECTION                                  //
    //  .   .   .   .   .   .   .   .   .   .   .   .   .   .   .   .   .   .   .   //
    connecterEnseignant(pPseudo, pMdp, callback)
    {
        let connection = mysql.createConnection(config);
        connection.connect();
        connection.query(`SELECT pseudo,mdp FROM enseignant WHERE pseudo = ? AND mdp = ?`,[pPseudo, pMdp], (error, results) => {
            callback(results, error);
        });
        connection.end();
    }

    connecterAdmin(pPseudo, pMdp, callback)
    {
        let connection = mysql.createConnection(config);
        connection.connect();
        connection.query(`SELECT pseudo,mdp FROM admin WHERE pseudo = ? AND mdp = ?`,[pPseudo, pMdp], (error, results) => {
            callback(results, error);
        });
        connection.end();
    }
